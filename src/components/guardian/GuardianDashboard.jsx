        ...
        {nav === "health" && !dataLoading && (
          <div className="s2">
            ...
            {healthEvents.some(e => e.event_type === "visual_analysis") && (() => {
              ...
              return (
                <div className="gcard" style={{ padding: isMobile ? 16 : 24, marginBottom: 14, animation: "fadeUp .5s ease .3s both" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
                    <div style={{
                      ...
                      <Scan size={17} color="#FFF8F0" />
                    </div>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: "#1a1a1a" }}>Visual Health Analysis</div>
                      <div style={{ fontSize: 11, color: "#6b6b6b", marginTop: 1 }}>Biometric markers from video recordings · {visualEvents.length} sample{visualEvents.length !== 1 ? "s" : ""}</div>
                    </div>
                  </div>
                  ...
                  {visualEvents[0]?.value?.priority_review && (
                    <div style={{
                      ...
                      <span style={{ fontSize: 12, fontWeight: 600, color: "#E53935" }}>Priority review flagged — check latest visual markers below</span>
                    </div>
                  )}
                        ...
                        <div key={marker.key} style={{
                          ...
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                              ...
                              <div>
                                <div style={{ fontSize: 12, fontWeight: 600, color: "#1a1a1a" }}>{marker.label}</div>
                          ...
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                            ...
                            {latest?.score != null && (
                              <span style={{ fontSize: 20, fontWeight: 700, color: "#1a1a1a" }}>{latest.score}%</span>
                            )}
                          </div>
                          {latest && (
                  ...
                  {visualEvents.length >= 2 && (
                    <div style={{ marginTop: 16, textAlign: "center" }}>
                      <button onClick={() => setCompareIdx(compareIdx != null ? null : 1)} style={{
                        padding: "8px 22px", borderRadius: 100, border: "1px solid rgba(93,64,55,0.15)",
                      ...
                      {compareIdx != null && visualEvents[compareIdx] && (
                        <div style={{
                          ...
                          <div style={{ fontSize: 11, fontWeight: 600, color: "#8D6E63", marginBottom: 10 }}>
                  ...
                  {(() => {
                    ...
                    const name = parentProfile?.full_name?.split(" ")[0] || "Parent";
                    const notes = [];
                    if (latest.skin_pallor?.label === "Pale" || latest.skin_pallor?.label === "Slightly pale")
                      notes.push(`${name} appears slightly pale compared to recent recordings.`);
                    if (latest.eye_engagement?.label === "Low engagement" || latest.eye_engagement?.label === "Unfocused")
                      notes.push(`Eye engagement seems lower — could be tiredness or lighting.`);
                    if (latest.motor_control?.label?.includes("tremor"))
                      notes.push(`Some motor control variation noted — may be worth monitoring.`);
                    if (notes.length === 0) return null;
                    return (
                      <div style={{ marginTop: 14, padding: "10px 14px", borderRadius: 12, background: "rgba(255,152,0,0.05)", border: "1px solid rgba(255,152,0,0.1)" }}>
                        {notes.map((n, i) => (
                          <p key={i} style={{ fontSize: 11, color: "#6b6b6b", lineHeight: 1.5, margin: i > 0 ? "6px 0 0" : 0 }}>
                            <span style={{ fontSize: 9, fontWeight: 600, color: "#FF9800", padding: "1px 7px", borderRadius: 100, background: "rgba(255,152,0,0.1)", marginRight: 6 }}>Observation</span>
                            {n}
                          </p>
                        ))}
                      </div>
